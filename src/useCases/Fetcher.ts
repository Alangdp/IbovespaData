...
export default class TickerFetcher {
  ...
  async getBasicInfo() {
    ...
    const image = this.Utility.extractImage(selectors.imageURL)
  ...
  async getStockrebuy() {
    ...
    for (const tableRow of tableRows) {
      const rowContent = cheerio.load(tableRow).html()
  ...
  static async getHighsAndLows() {
    ...
    try {
      ...
      listItems.each((i, item) => {
        ...
        const currentPrice = $item.find('.main-info .other-value').text().trim()
  ...
  private static async getNewContent(news: News[]) {
    ...
    for (const item of news) {
      ...
      const response = await axios.get(`https://br.tradingview.com${item.link}`)
      const $ = cheerio.load(response.data)
      const content = $(
