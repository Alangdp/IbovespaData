...
interface IPontuationService {
  createPontuationInstances(
    pontuationData: Record<string, PontuationProps>,
  ): string
}
...
class PontuationService implements IPontuationService {
  ...
  public createPontuationInstances(
    pontuationData: Record<string, PontuationProps>,
  ): string {
    ...
    Object.keys(pontuationData).forEach((key) => {
      ...
      this.rules.forEach((ruleName) => {
        const rule = instance.totalEvaluate.find(
...
class FileService {
  public static readJSONFromFile(
    filePath: string,
  ): Record<string, PontuationProps> {
